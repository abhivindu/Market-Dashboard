...
## Features
- **Chart coverage limited to the "featured" set** — interactive price charts (Chart.js) only exist for the ~52 tickers that actually appeared in a gainers/losers/volume-outliers/recommendations pull (`pipeline/fetch_featured_history.py`), plus the 5 indices. Any other name in the sector member tables or a manually-added portfolio ticker shows "no chart history available" until it happens to surface in a future pull. Fixing this properly means either fetching 1y history for the full ~1500-name universe (slower, bigger payload) or fetching on-demand per ticker when a user clicks — the latter needs a live capability, not just a bigger pre-bake.
